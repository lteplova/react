...
const ChartsSerie = (props) => {
  return (
    <div
      className={`Charts--serie ${props.type}`}
        ...
        var color = props.colors[itemIndex],
          style,
					...
					if (props.type == 'layered') {
						style = Object.assign(style, {right: (props.sortedSerie.indexOf(item) / (props.serie.length + 1)) * 100 + "%"});
					}
  ...
  render() {
    ...
    return (
      <section>
				...
				<Charts dir="horizontal" max={max} data={data} colors={colors} labels={labels} height={'auto'} />
